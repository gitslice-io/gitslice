    ...
    <div className="rounded-lg border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm shadow-slate-200/50 dark:shadow-black/50">
      <div className="px-3 py-2.5 md:px-5 md:py-3">
        ...
        <div className="flex flex-col gap-2.5 lg:flex-row lg:items-start lg:justify-between lg:gap-3">
          <div className="min-w-0">
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 md:gap-3">
              <h1
                className="truncate text-base font-semibold tracking-normal text-zinc-950 dark:text-zinc-50 sm:text-lg md:text-xl"
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-x-1.5 gap-y-1 text-[11px] text-slate-600 dark:text-zinc-400 md:mt-2 md:gap-x-2 md:gap-y-1.5 md:text-xs">
              <span
                className="max-w-[12rem] truncate rounded bg-slate-100 dark:bg-zinc-800 px-1.5 py-0.5 font-mono text-slate-700 dark:text-zinc-300 md:px-2 md:py-1"
              ...
              {changeset.parentChangesetId ? (
                <Link
                  className="inline-flex max-w-[12rem] items-center gap-1 truncate rounded border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-1.5 py-0.5 font-medium text-slate-600 dark:text-zinc-400 transition hover:border-slate-300 dark:hover:border-zinc-700 hover:text-zinc-950 dark:hover:text-zinc-50 md:max-w-[14rem] md:px-2 md:py-1"
                  params={{
                    id:
        ...
        {changeset.submitBlockedReason ? (
          <div className="mt-2.5 rounded-md border border-amber-200 dark:border-amber-900/60 bg-amber-50 dark:bg-amber-950/30 px-3 py-2 text-xs text-amber-900 dark:text-amber-200 md:mt-3 md:text-sm">
      ...
      {patchsetCompare ? (
        <div className="border-t border-slate-100 dark:border-zinc-800 px-3 py-2 md:px-5 md:py-2.5">
          {patchsetCompare}
        </div>
      ) : null}
    </div>
